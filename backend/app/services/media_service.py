...
def _encode_grouped_cursor(sort_path: str, kind: str, entity_id: int) -> str:
    payload = json.dumps(
        {"sort_path": sort_path, "kind": kind, "entity_id": entity_id},
        separators=(",", ":"),
        ensure_ascii=True,
    ).encode("utf-8")
    return base64.urlsafe_b64encode(payload).decode("ascii").rstrip("=")
...
def _decode_grouped_cursor(value: str | None) -> dict | None:
    if not value:
        return None
    try:
        padding = "=" * (-len(value) % 4)
        ...
        return None
    if not isinstance(decoded, dict) or "sort_path" not in decoded or "kind" not in decoded or "entity_id" not in decoded:
        return None
    return decoded
...
def _build_filtered_library_file_ids_subquery(
    db: Session,
    library_id: int,
    *,
    search: str = "",
    ...
    sort_key: FileSortKey = "file",
    sort_direction: FileSortDirection = "asc",
    name: str = "filtered_library_file_ids",
):
    return (
        _build_library_file_id_query(
            db,
            library_id,
            search=search,
...
def _csv_export_row(row: MediaFileTableRow) -> list[str | int | float]:
    values = [
        ...
        row.audio_artist or "",
        row.audio_album or "",
        row.audio_album_artist or "",
        row.audio_genre or "",
        row.audio_date or "",
        row.audio_disc or "",
        row.audio_composer or "",
        _stringify_export_scalar(row.audio_channels),
        _stringify_export_scalar(row.sample_rate),
        row.track_number or "",
...
def _average_present(values: list[float | int | None]) -> float | None:
...
def list_grouped_library_files(
    db: Session,
    library_id: int,
    *,
    offset: int = 0,
    ...
    filtered_ids = _build_filtered_library_file_ids_subquery(
        db,
        library_id,
        search=search,
        search_filters=search_filters,
        sort_key="file",
        sort_direction=sort_direction,
        name="grouped_filtered_library_file_ids",
    )
    filtered_media_files = (
        select(
            MediaFile.id.label("id"),
            MediaFile.relative_path.label("relative_path"),
            MediaFile.size_bytes.label("size_bytes"),
            MediaFile.duration_seconds.label("duration_seconds"),
            MediaFile.quality_score.label("quality_score"),
            MediaFile.bitrate.label("bitrate"),
            MediaFile.audio_bitrate.label("audio_bitrate"),
            MediaFile.series_id.label("series_id"),
            MediaFile.season_id.label("season_id"),
            MediaFile.last_analyzed_at.label("last_analyzed_at"),
        )
        .join(filtered_ids, filtered_ids.c.id == MediaFile.id)
        .subquery("grouped_filtered_media_files")
    )
    series_rows = select(
        literal("series").label("kind"),
        func.lower(MediaSeries.relative_path).label("sort_path"),
        MediaSeries.id.label("entity_id"),
        MediaSeries.id.label("series_id"),
        MediaSeries.title.label("title"),
        MediaSeries.relative_path.label("relative_path"),
        MediaSeries.year.label("year"),
        func.count(filtered_media_files.c.id).label("episode_count"),
        func.count(func.distinct(filtered_media_files.c.season_id)).label("season_count"),
        ...
        MediaSeries.id,
        MediaSeries.title,
        MediaSeries.relative_path,
        MediaSeries.year,
    )
    loose_file_rows = select(
        literal("file").label("kind"),
        func.lower(filtered_media_files.c.relative_path).label("sort_path"),
        filtered_media_files.c.id.label("entity_id"),
        literal(None).label("series_id"),
        literal(None).label("title"),
        filtered_media_files.c.relative_path.label("relative_path"),
        literal(None).label("year"),
        literal(None).label("episode_count"),
        literal(None).label("season_count"),
        literal(None).label("total_size_bytes"),
        literal(None).label("total_duration_seconds"),
        literal(None).label("quality_score_average"),
        literal(None).label("bitrate_average"),
        literal(None).label("audio_bitrate_average"),
        filtered_media_files.c.id.label("file_id"),
    ).select_from(filtered_media_files).where(filtered_media_files.c.series_id.is_(None))
    grouped_entries = union_all(series_rows, loose_file_rows).subquery("grouped_media_entries")
    grouped_query = select(grouped_entries)
    total = (
        (db.scalar(select(func.count()).select_from(grouped_query.subquery())) or 0)
        if include_total
        else None
    )
    cursor_payload = _decode_grouped_cursor(cursor)
    if cursor_payload:
        cursor_sort_path = str(cursor_payload["sort_path"])
        cursor_kind = str(cursor_payload["kind"])
        cursor_entity_id = int(cursor_payload["entity_id"])
        if sort_direction == "desc":
            grouped_query = grouped_query.where(
                or_(
                    grouped_entries.c.sort_path < cursor_sort_path,
                    and_(grouped_entries.c.sort_path == cursor_sort_path, grouped_entries.c.kind < cursor_kind),
                    and_(
                        grouped_entries.c.sort_path == cursor_sort_path,
                        grouped_entries.c.kind == cursor_kind,
                        grouped_entries.c.entity_id < cursor_entity_id,
                    ),
                )
            )
        else:
    ...
    grouped_query = grouped_query.order_by(
        grouped_entries.c.sort_path.desc() if sort_direction == "desc" else grouped_entries.c.sort_path.asc(),
        grouped_entries.c.kind.desc() if sort_direction == "desc" else grouped_entries.c.kind.asc(),
        grouped_entries.c.entity_id.desc() if sort_direction == "desc" else grouped_entries.c.entity_id.asc(),
    )
    page_rows = db.execute(grouped_query.offset(offset).limit(limit + 1)).mappings().all()
    has_more = len(page_rows) > limit
    page_rows = page_rows[:limit]
    if not page_rows:
        return GroupedMediaTablePageRead(total=total, offset=offset, limit=limit, items=[], has_more=False)
    ...
    for row in page_rows:
        if row["kind"] == "series":
            ...
            items.append(
                GroupedSeriesTableRowRead(
                    ...
                    total_size_bytes=int(metrics.get("total_size_bytes") or 0),
                    total_duration_seconds=float(metrics.get("total_duration_seconds") or 0),
        ...
        file_row = loose_files.get(file_id)
        if file_row is None:
            continue
        items.append(GroupedLooseFileTableRowRead(kind="file", file=file_row))

    last_row = page_rows[-1]
    next_cursor = (
        _encode_grouped_cursor(str(last_row["sort_path"]), str(last_row["kind"]), int(last_row["entity_id"]))
        if has_more
        else None
    )
    return GroupedMediaTablePageRead(
        total=total,
        offset=offset,
        limit=limit,
        next_cursor=next_cursor,
...
def get_grouped_library_series_detail(
    db: Session,
    library_id: int,
    series_id: int,
    *,
    search: str = "",
    search_filters: LibraryFileSearchFilters | None = None,
) -> MediaSeriesGroupedDetailRead | None:
    series = db.scalar(select(MediaSeries).where(MediaSeries.library_id == library_id, MediaSeries.id == series_id))
    if series is None:
        return None
    ...
    filtered_ids = _build_filtered_library_file_ids_subquery(
        ...
        sort_key="file",
        sort_direction="asc",
        name="grouped_series_detail_filtered_ids",
    )
    selected_ids = list(
        db.scalars(
            select(MediaFile.id)
            .join(filtered_ids, filtered_ids.c.id == MediaFile.id)
            .where(MediaFile.series_id == series_id)
            .order_by(MediaFile.season_id.asc(), MediaFile.episode_number.asc(), MediaFile.relative_path.asc())
        ).all()
    )
    if not selected_ids:
        return None
    ...
    for row in rows:
        if row.season_id is None:
            ...
            continue
        rows_by_season_id.setdefault(row.season_id, []).append(row)
    ...
    season_payloads: list[MediaSeasonDetailRead] = []
    for season in seasons:
        season_rows = rows_by_season_id.get(season.id, [])
        if not season_rows:
            continue
        season_payloads.append(
            MediaSeasonDetailRead(
                ...
                title=season.title,
                relative_path=season.relative_path,
                episode_count=len(season_rows),
                total_size_bytes=sum(row.size_bytes for row in season_rows),
                total_duration_seconds=sum(row.duration or 0 for row in season_rows),
                episodes=season_rows,
            )
        )

    summary = MediaSeriesSummaryRead(
        id=series.id,
        library_id=series.library_id,
        title=series.title,
        normalized_title=series.normalized_title,
        relative_path=series.relative_path,
        year=series.year,
        season_count=len(season_payloads),
        episode_count=len(rows),
        total_size_bytes=sum(row.size_bytes for row in rows),
        total_duration_seconds=sum(row.duration or 0 for row in rows),
        last_analyzed_at=max((row.last_analyzed_at for row in rows if row.last_analyzed_at), default=None),
    )
    return MediaSeriesGroupedDetailRead(
        **summary.model_dump(),
        seasons=season_payloads,
        episodes_without_season=rows_without_season,
    )
