...
test("installer URLs only allow expected HTTPS latest-release assets", () => {
  assert.equal(
    isAllowedInstallerDownloadUrl(
      "https://github.com/NPontious/MediaLyze/releases/latest/download/MediaLyze.AppImage"
    ),
    true
  );
  assert.equal(isAllowedInstallerDownloadUrl("http://github.com/NPontious/MediaLyze/releases/latest/download/MediaLyze.AppImage"), false);
  assert.equal(isAllowedInstallerDownloadUrl("https://example.test/NPontious/MediaLyze/releases/latest/download/MediaLyze.AppImage"), false);
  assert.equal(isAllowedInstallerDownloadUrl("https://github.com/NPontious/MediaLyze/releases/latest/download/evil.exe"), false);
});
...
test("download descriptors use stable URLs and versioned local filenames", () => {
  assert.deepEqual(buildLatestInstallerDownload("darwin", "0.12.0"), {
    ...
    url: "https://github.com/NPontious/MediaLyze/releases/latest/download/MediaLyze-arm64.dmg",
